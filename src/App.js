...
function App() {
  ...
  const fetchTasks = async (token) => {
    const response = await fetch("https://taskventures.onrender.com/tasks", {
  ...
  const deleteTask = async (id) => {
    await fetch(`https://taskventures.onrender.com/tasks/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const updateTaskStatus = async (id, currentStatus) => {
    ...
    const response = await fetch(
      `https://taskventures.onrender.com/tasks/${id}/status`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const updateTaskPriority = async (id, newPriority) => {
    const response = await fetch(
      `https://taskventures.onrender.com/tasks/${id}/priority`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
      ...
      <main className="flex-1 p-8">
        <h1 className="text-4xl font-extrabold text-center mb-8 text-orange-600 drop-shadow">
          <div className="text-center mt-10">
            <h1 className="text-4xl font-bold text-orange-600">TASKVENTURE</h1>
            <p className="text-lg text-gray-600 mt-2 italic">
              Explore it, complete it, and win your day
            </p>
          </div>
        </h1>
        <form
